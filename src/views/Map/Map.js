...
export default class Map extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            lng: 144.96332,
            zoom: 13,
            routes: [2, 3],    // 2 = Belgrave, 3 = Cragieburn, 14 = Sunbury, 15 = Upfield, 
            stops: [],
            departures: [],
            latlngs: [],
            rails: [],
    ...
    componentDidMount() {
        ...
        // setInterval(() => {
        //     API.healthCheck();
        //     API.getDeparturesForRoute(3, this.state.stops)
        //         .then(response => {
        //             this.setState({
        //                 departures: response
        //             })
    ...
    render() {
        ...
        if (departures.length > 0) {
            console.log(departures);
            runs = Departures.getUniqueRuns(departures, 3);
            filteredRuns = Departures.getDeparturesForRuns(runs, departures);
            console.log(filteredRuns);
            for (let i in filteredRuns) {
                if (filteredRuns[i].departures[0].at_platform) {
                    runsAtStation.push({
                        departure: filteredRuns[i].departures[0],
                        coordinates: Stations.getStopCoordinate(stations, filteredRuns[i].departures[0].stop_id)
                    });
                } else {
                    runsBetweenStations.push({
                        departure: filteredRuns[i].departures[0],
                        coordinates: Stations.getCoordinatesPair(stations, filteredRuns[i].departures[0].stop_id, filteredRuns[i].direction_id)
                    });
                }
            }
        }
